...
public interface MansionAvailablePeriodRepository extends JpaRepository<MansionAvailablePeriod,Long>{
    ...
    @Query(value = "SELECT available_period_id,start_date,end_date,mansion_id FROM public.mansion_available_period u WHERE u.start_date <= :startDate AND u.end_date >= :endDate",
    		nativeQuery = true)
    MansionAvailablePeriod getPeriodOfInterest(@Param("startDate") Date startDate,@Param("endDate") Date endDate);
