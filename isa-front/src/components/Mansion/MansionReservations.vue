<template>
  <div class="mansion-reservations" v-if="loggedUser!=null && loggedUser.advertiserType!=null && loggedUser.advertiserType=='mansion'">
    ...
    <nav class="navbar navbar-dark bg-dark" style=" height: 10%;">
      ...
      <input class="form-control me-2" type="search" placeholder="Search mansion" v-model="filterMansionName" aria-label="Search" style="width: 200px">
    ...
    <div v-if="typeOfReservation=='regular'">
      <table class="table table-striped table-hover">
        ...
        <tr v-for="(reservation, index) in filteredReservations" :key="reservation.id" style="align-content: center"
            v-on:click="selectReservation(reservation, 'REGULAR_MANSION')" data-toggle="modal" data-target="#exampleModal">
          <th scope="row" style="align-content: center" >{{index+1}}</th>
          <td>{{reservation.mansion.name}}</td>
      ...
      <div class="modal-dialog">
        <div class="modal-content" style="padding: 5%" v-if="selectedReservation!=null">
          ...
          <table class = "table table-borderless">
            <tbody>
            <tr>
              <td class="row">Mansion name</td>
              <td>{{this.selectedReservation.mansion.name}}</td>
            </tr>
            <tr>
...
export default {
  ...
  data: function (){
    return{
      ...
      selectedReservationType: 'REGULAR_MANSION',
    ...
    axios.get(devServer.proxy + "/userData", {
      headers: {
        ...
        .then(response => {
              ...
              console.log("Ovaj user je ulogovan Mansion Reservations:", this.loggedUser)
              if(this.loggedUser.advertiserType == 'mansion'){
                axios.get(devServer.proxy+"/getMansionOwnerReservations", {
                  headers: {
                    'Authorization' : this.$store.getters.tokenString
                  }
                })
                    .then(response1 => {
                      console.log("Rezervacija vikrndica vlasnika", response1.data)
                      this.mansionReservations = response1.data.mansionReservations;
                      this.filteredReservations = this.mansionReservations
                      this.selectedReservation = this.mansionReservations[0]
                      this.discountReservations = response1.data.mansionDiscountReservations
  ...
  methods:{
    ...
    reservationFilter(){
      ...
      console.log("filter reservation params:", this.filterMansionName, "start", this.startDateTime,"end", this.endDateTime, "status", this.filterReservationStatus)

      if(this.typeOfReservation === 'regular'){
        this.filteredReservations = new Array()
        for (var reservation of this.mansionReservations){
          console.log("prvi uslov:", reservation.mansion.name.includes(this.filterMansionName),
          ...
          if(reservation.mansion.name.includes(this.filterMansionName)
        ...
        for (var dreservation of this.discountReservations){
          if(dreservation.mansion.name.includes(this.filterMansionName)
