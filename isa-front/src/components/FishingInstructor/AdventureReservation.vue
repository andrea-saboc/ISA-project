<template>
  <div class="boat-reservations" v-if="loggedUser!=null && loggedUser.advertiserType!=null && loggedUser.advertiserType=='fishing'">
    ...
    <nav class="navbar navbar-dark bg-dark" style=" height: 10%;">
      ...
      <input class="form-control me-2" type="search" placeholder="Search adventure" v-model="filterAdventureName" aria-label="Search" style="width: 200px">
    ...
    <div v-if="typeOfReservation=='regular'">
      <table class="table table-striped table-hover">
        ...
        <tr v-for="(reservation, index) in filteredReservations" :key="reservation.id" style="align-content: center"
            v-on:click="selectReservation(reservation, 'REGULAR_ADVENTURE')" data-toggle="modal" data-target="#exampleModal">
    ...
    <div v-if="typeOfReservation==='discount'">
      <table  class="table table-striped table-hover">
        ...
        <tr v-for="(reservation, index) in filteredDiscountReservations" :key="reservation.id" style="align-content: center"
            v-on:click="selectReservation(reservation, 'DISCOUNT_ADVENTURE')" data-toggle="modal" data-target="#exampleModal">
          <th scope="row" style="align-content: center" >{{index+1}}</th>
          <td>{{reservation.adventure.name}}</td>
          <td v-bind:id="reservation.id+'resid'" style="align-content: center">{{formatDate(reservation.startDate)}}</td>
          <td style="align-content: center">{{formatDate(reservation.endDate)}}</td>
      ...
      <div class="modal-dialog">
        <div class="modal-content" style="padding: 5%" v-if="selectedReservation!=null">
              ...
              <tr>
                ...
                <td>
                  <div v-for="subsrciption in selectedUserSubscribed" :key="subsrciption.id" >
                    <span v-if="subsrciption.adventure.fishingInstructor.id == loggedUser.id" class="badge bg-success" style="color: #eeeeee">{{subsrciption.adventure.name}}</span>
                    <span v-else class="badge bg-dark" style="color: #eeeeee">{{subsrciption.adventure.name}}</span>
                  </div>
                </td>
              </tr>
              </tbody>
          ...
          <table class = "table table-borderless">
            <tbody>
            <tr>
              <td class="row">Adventure name</td>
              <td>{{this.selectedReservation.adventure.name}}</td>
            </tr>
            <tr>
...
export default {
  ...
  data: function (){
    return{
      ...
      selectedReservationType: 'REGULAR_ADVENTURE',
          ...
          if(this.loggedUser.advertiserType == 'fishing'){
            axios.get("/getFishingInstructorReservations", {
              headers: {
                ...
                .then(response1 => {
                  console.log("Rezervacija avantura vlasnika", response1.data)
                  this.adventureReservations = response1.data.adventureReservations;
                  this.filteredReservations = this.adventureReservations
                  this.selectedReservation = this.adventureReservations[0]
                  this.discountReservations = response1.data.adventureDiscountReservations;
  ...
  methods:{
        ...
        axios.get("/subscriptions/adventureByFishingInstructor", {
          ...
          params:{
            ...
            .then(respone => {
              ...
              console.log("Selected user is subscribed at adventures: ", this.selectedUserSubscribed)
            })
            .catch(() =>{
              console.log("Subscriber adventures are unavailable")
            })
      } else{
    ...
    reservationFilter(){
      ...
      console.log("filter reservation params:", this.filterAdventureName, "start", this.startDateTime,"end", this.endDateTime, "status", this.filterReservationStatus)

      if(this.typeOfReservation === 'regular'){
        this.filteredReservations = new Array()
        for (var reservation of this.adventureReservations){
          console.log("prvi uslov:", reservation.adventure.name.includes(this.filterAdventureName),
        ...
        "cetvrti uslov", (new Date(reservation.endDate).getTime() <= (new Date(this.endDateTime).getTime()))  || this.endDateTime=='')
          if(reservation.adventure.name.includes(this.filterAdventureName)
        ...
        for (var dreservation of this.discountReservations){
          if(dreservation.adventure.name.includes(this.filterAdventureName)
